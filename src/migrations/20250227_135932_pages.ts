...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  CREATE TABLE IF NOT EXISTS "users" (
  	...
  	"cart_items_price" numeric,
  	"cart_tax_price" numeric,
  	"cart_shipping_price" numeric,
  	"cart_total_price" numeric,
  	"cart_payment_method" varchar,
  	"cart_shipping_address" varchar,
  	"cart_delivery_date_index" timestamp(3) with time zone,
  	"cart_deliverydateindex_tz" "enum_users_cart_deliverydateindex_tz" DEFAULT 'Africa/Lagos',
  	"cart_expected_delivery_date" timestamp(3) with time zone,
  	"cart_expecteddeliverydate_tz" "enum_users_cart_expecteddeliverydate_tz" DEFAULT 'Africa/Lagos',
  	"skip_sync" boolean,
  	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
  	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
  ...
  CREATE TABLE IF NOT EXISTS "orders" (
  	...
  	"expecteddeliverydate_tz" varchar NOT NULL,
