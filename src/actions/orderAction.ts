...
// export const createOrderFromCart = async (clientSideCart: Cart, userId: string) => {
//   //console.log('createOrderFromCart called with cart:', clientSideCart)
...
//     // console.log(
//     //   `Converting product ID: ${item.product} (type: ${typeof item.product}) to ${productId} (type: ${typeof productId})`,
//     // )
...
//  // console.log('Cart after calcDeliveryDateAndPrice:', cart)
...
//      items: clientSideCart.items.map((item) => {
//        // Convert `product` from string to number
//        const productId = typeof item.product === 'string' ? Number(item.product) : item.product
...
//   console.log('Cart items after conversion:', cart.items)

//   // Convert userId to a number
//   const userIdNumber = parseInt(userId, 10)

//   // Step 2: Validate input using your schema
//   const orderData = OrderInputSchema.parse({
//     user: userIdNumber, // Use the converted userId
//     // items: cart.items,
//     items: cart.items.map((item) => ({
//       ...item,
//       product: typeof item.product === 'string' ? Number(item.product) : item.product,
...
//     totalPrice: cart.totalPrice,
//     expectedDeliveryDate: cart.expectedDeliveryDate
//       ? cart.expectedDeliveryDate.toISOString() // Convert to string
//       : new Date().toISOString(), // Or handle undefined as an error
//   })

//   console.log('Validated order data:', orderData)

//   // Step 3: Create order in Payload collection
//   const order = await payload.create({
//     collection: 'orders', // Name of your orders collection
//     data: orderData,
//   })
...
//       items: clientSideCart.items.map((item) => {
//         const productId =
//           typeof item.product === 'string' ? parseInt(item.product, 10) : item.product
//         console.log(`Converting product ID: ${item.product} to ${productId}`)
//         return {
//           ...item,
//           product: productId,
//         }
...
//       items: clientSideCart.items.map((item) => ({
//         ...item,
//         product: typeof item.product === 'string' ? parseInt(item.product, 10) : item.product,
//       })),
//       shippingAddress: clientSideCart.shippingAddress,
//       deliveryDateIndex: clientSideCart.deliveryDateIndex,
//     }),
